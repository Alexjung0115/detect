...
async def process_video(file: UploadFile = File(...)):
    ...
    try:
        ...
        # AI 처리 (단순 복사 후 infer 호출)
        shutil.copy(upload_path, processed_path)

        # Run inference and get event logs
        event_logs = infer(processed_path)

        # ffmpeg 명령어 실행 (동영상 포맷 변환)
        output_path = os.path.join("static", "processed", "overlay_video_h264.mp4")
        ffmpeg_command = [
            "ffmpeg",
            "-i", "/home/alpaco/osh/final/overlay_video.mp4",  # 입력 파일 경로
            "-c:v", "libx264",                  # 비디오 코덱
            "-preset", "slow",                  # 인코딩 속도
            "-crf", "23",                       # 비디오 품질 설정
